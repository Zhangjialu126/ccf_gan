...
class Discriminator(nn.Module):
    ...
    def __init__(self, D_ch=64, D_wide=True, resolution=128,
        ...
        self.unconditional = kwargs['unconditional']
        self.tx_lambda = kwargs['tx_lambda']
        # Which convs, batchnorms, and linear layers to use
        ...
        if not self.unconditional:
            ...
            self.embed = nn.Embedding(self.n_classes, 128)
            self.linear_r = nn.Linear(self.arch['out_channels'][-1], self.n_classes)
            self.linear_f = nn.Linear(self.arch['out_channels'][-1], self.n_classes)
    ...
    def forward(self, x, y=None):
        ...
        out = self.linear(h)
        out = self.tanh(out)
        out = out * self.tx_lambda
        if self.unconditional:
            return out
        else:
            return out, h

    def y_embedding(self, y=None):
        y_embed = self.embed(y)
        return y_embed, 128

    def classifier(self, h):
        label_out_r = self.linear_r(h)
        label_out_f = self.linear_f(h)
        return label_out_r, label_out_f
